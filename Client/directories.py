...
from tkinter import Label, Entry, Button, Listbox, Scrollbar
import json
import time
...
class DirectoryGUI:
    ...
    def create_choose_directory_page(self):
        ...
        Button(self.frame, text="Back to Menu", font=("Arial", 16, "bold"), bg="#6c757d", fg="white", width=40, height=2, bd=0, command=self.create_main_menu).grid(row=4, column=0, columnspan=2, pady=(15, 0))
    ...
    def handle_directory_selection(self):
        ...
        try:
            ...
            if "successfully" in response.lower():
                ...
                self.frame.after(1500, lambda: self.client_socket.send("BACK_TO_MENU".encode('utf-8')))
    ...
    def create_directory_page(self):
        ...
        self.dir_entry.grid(row=1, column=0, pady=(0, 15), padx=5)
        Button(self.frame, text="Create", font=("Arial", 16, "bold"), bg="#17a2b8", fg="white", width=10, height=1, bd=0, command=self.handle_create_directory).grid(row=1, column=1, pady=(0, 15), padx=5)
        self.dir_output_label = Label(self.frame, text="", font=("Arial", 14), fg="black", bg="white")
        self.dir_output_label.grid(row=2, column=0, columnspan=2, pady=(5, 15))
        Button(self.frame, text="Back to Menu", font=("Arial", 16, "bold"), bg="#6c757d", fg="white", width=40, height=2, bd=0, command=self.create_main_menu).grid(row=3, column=0, columnspan=2, pady=(15, 0))
    ...
    def create_user_management_page(self):
        ...
        Button(self.frame, text="Back to Menu", font=("Arial", 16, "bold"), bg="#6c757d", fg="white", width=40, height=2, bd=0, command=lambda: [self.client_socket.send("5".encode()), self.create_main_menu()]).grid(row=6, column=0, columnspan=2, pady=(15, 0))
        try:
    ...
    def handle_add_user(self):
        selected_dir = self.dir_listbox.get(self.dir_listbox.curselection()) if self.dir_listbox.curselection() else None
